...
| 2026-08-28 | SDK browser build + WalletSigner | ✅ | Added `marc-stellar-sdk/browser` subpath (esbuild ESM bundle, ~1 MB minified + sourcemap, zero Node/@x402 imports) via `sdk/src/browser.ts` + `tsconfig.browser.json` + `build:browser` script + `exports` map. Added `sdk/src/signer.ts`: `WalletSigner` interface, `KeypairSigner` adapter, `toSigner`/`signerPublicKey` helpers. `IdentityClient`/`CommerceClient` methods now accept `Signer = Keypair | WalletSigner` — fully backward compatible (demo/dashboard/agents typecheck clean). Enables dashboard client-side wallet signing (Freighter/SWK). **Pre-existing, unfixed:** `demo/buyer-agent.ts` references undefined `sellerPubkey`/`sellerPort` and `demo/seller-agent.ts` passes a `number` jobId to `submit` — unrelated to SDK, present before this change. |
...
- `@stellar/stellar-sdk` 14.x's `browser` package field points to the legacy UMD bundle (`dist/stellar-sdk.min.js`). esbuild resolves it via the `browser` condition in the `exports` map — `--main-fields`/`--conditions` do NOT override it (the CJS `lib/` build requires Node builtins: `path`, `axios`, `commander`, `eventsource`, so it's not a browser alternative). The UMD wrapper references the `self` global: fine in browsers, but a Node smoke-test of the browser bundle needs `globalThis.self = globalThis` first. Verdict: the UMD bundle IS the right browser artifact — bundle it, don't fight it.
- To smoke-test a package's `exports`-map subpath (e.g. `marc-stellar-sdk/browser`) from Node: run the test script from INSIDE the package dir — Node's package self-reference resolves the package by its own `name` when the importing file is within the package. A script in `/tmp` won't resolve it.
- Adding a devDependency to the SDK (esbuild) shows up as a 1-line `"esbuild": ...` addition in every consumer's package-lock.json (demo, dashboard, agents) on next `npm install`. Benign — it's npm recording the `file:` dependency's metadata.
